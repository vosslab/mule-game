...
| [walker_debug.ts](../src/ui/walker_debug.ts)   | `buildWalkerProjection` (walkthrough-harness): builds the read-only `window.muleGameState()` snapshot, a `structuredClone`d and deep-frozen plain-data copy of `GameState` plus convenience fields (`phaseKind`, `activePlayerId`, `humanMoney`, `sweepRow`, `sweepCol`). The browser-driven walker orchestrator reads only the convenience fields; the strategy adapter (`tests/e2e/walkthrough_strategy.mjs`) consumes the full state, since it calls the same `src/ai/` decide functions the engine does.        |
...
| [town_scene.tsx](../src/ui/scenes/town_scene.tsx)                   | Town scene shell: rAF movement/camera loop and panel wiring over the mode-composed, horizontally-scrolling street built by `composeTownStreetForMode` (`town_world.ts`) and scrolled by `town_camera.ts`; presentational SVG lives in `town_scene_render.tsx`, the interaction state machine in `town_interaction.ts`, and facade collision/door-open state in `town_collision.ts`. Two street-endpoint exits, corral spawn, and walk-in-then-confirm door entry; replaced the develop-turn store menu.                                                                                                                                                                                                                                                             |
