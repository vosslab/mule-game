...
- Testing (Patch 58, WP-6C): fixed a walker-harness gesture bug and two stale
  ...
  and `e2e_full_game` (`tests/e2e/`) were updated off the pre-WP-4B
...
- Testing (Patch 59, WP-6D): removed `tests/e2e/e2e_walk_calibration.mjs`. Its
...
- Testing (Patch 57, WP-6B): locked the town spacing constants (unchanged: gap
  ...
  [docs/active_plans/audits/town_spacing_experiment.md](active_plans/audits/town_spacing_experiment.md).
- Testing (Patch 58, WP-6C): the walkthrough sweep was re-triaged and RESTORED
...
- Testing (Patch 57, WP-6B): the spacing/travel-budget experiment measured the
...
- UI (Patch 10, WP-3A): town facades rebuilt in `src/ui/scenes/town_scene.tsx`
...
- UI (Patch 11, WP-3B): town facades now show live ambient economics in
  ...
  labels (Land Office pricing deferred to WP-5B). Every value reads from the SAME
...
- UI (Patch 1-3, WP-1A/1B/1C): new `src/ui/scenes/town_world.ts` replaces the
  ...
  carries a fenced `TODO(WP-2B)` tsc-green shim with inert collision pending the
  camera cutover (see Decisions and Failures).
- UI (Patch 6, WP-2A): new `src/ui/scenes/town_camera.ts` -- a pure horizontal
...
- UI (Patch 8, WP-2C): new `src/ui/solid/town_chrome.tsx` -- a dedicated town
  ...
  label is a stub pending WP-3B/4A wiring.
- UI (Patch 13, WP-4A): `src/ui/scenes/town_scene.tsx` replaced the scattered
  ...
  behind a placeholder confirm panel (TODO WP-5A); land and assay route to
  placeholder/confirm panels (TODO WP-5B). Adds `data-town-state`. Reviewed
  PASS: attempt-then-confirm is enforced structurally, with no economic dispatch
  on entry anywhere.
- UI (Patch 52, WP-5B): shipped the town's Land Office and Assay Office
  transaction panels, replacing the retired WP-4A placeholder action-panel
...
- UI (Patch 7, WP-2B): `src/ui/scenes/town_scene.tsx` rewritten to render the
  ...
  immediate dispatch (TODO WP-5A), the Land Office is a neutral notice (TODO
  WP-5B), and endpoint exits map left->west/right->east until WP-4B. Reviewed
...
- UI (Patch 14, WP-4B): human develop turns now START in town at the corral
...
- UI (Patch 52, WP-5C): relocated the town `End turn` control out of
...
- UI (Patch 55, town modularization): split the two largest town modules into
...
- Docs (Patch 5, WP-1F): `docs/THE_TOWN_ANALYSIS.md` amended from a fixed
...
- UI (Patch 14, WP-4B): fixed a resource leak in
...
- UI (Patch 53, WP-5C): rewrote the stale develop and in-town tutorial hints to
  ...
  small chrome-strip `End turn` control. Removed the resolved `TODO(WP-5C)`
  comment from `game_screen.tsx`.

### Removals and Deprecations

- UI (WP-1A/1D): deleted `src/ui/scenes/town_layout.ts` and
  `tests/test_town_layout.mjs`, superseded by `src/ui/scenes/town_world.ts` and
  `tests/test_town_world.mjs`.
- Testing (Patch 9, WP-2D): retired the obsolete grid-topology specs
...
- Testing (Patch 54, WP-5D): retired (deleted)
...
- UI (Patch 12, WP-3C): recalibrated the `visual_render` "town scene fixture"
...
- UI (Patch 12, WP-3C polish): non-blocking items logged rather than blocking the
...
- UI (Patch 1-3, WP-2B interim state): retiring the 9x5 model left
  `src/ui/scenes/town_scene.tsx` on a fenced `TODO(WP-2B)` shim with inert
  collision to keep `tsc` green until the camera cutover lands; the interim
  state is accepted and tracked to WP-2B.
- UI (Patch 4, WP-1E, HIGH review finding): a quality review caught an
...
- Testing (Patch 9, WP-2D): the facade-label legibility ladder is scoped to the
...
- UI (Patch 13, WP-4A): the `at-threshold` state is modeled but effectively
...
- Testing (Patch 15, WP-4C): the six-spec town-first navigation fix used
  per-spec edits (no shared helper), matching WP-4C precedent, because the
  land-claim and post-turn steps differ across specs; a shared `develop_nav.mjs`
  helper is a possible future refactor.
- Testing (Patch 15, WP-4C): `corral_purchase`'s occasional full-parallel flake
  is a pre-existing land-grant sweep-cursor timing race under CPU contention,
  not town-caused.
- UI (Patch 52, WP-5B): modeled the Land Office as a one-member `informational`
...
- Testing (Patch 56, WP-6A): rebuilt the E2E walker town executors
...
- Testing (Patch 12, WP-3C): filed an automated visual-acceptance report at
...
- Testing (Patch 4, WP-1D/1E): new `tests/test_town_world.mjs` (16 cases):
  ...
  property test over capability-flag combinations, the WP-1E negative
...
- Testing (Patch 9, WP-2D): new `tests/playwright/town_street.spec.mjs` (7
  ...
  `tests/playwright/town_scene.spec.mjs` are marked `test.skip` with
  TODO(WP-5D). Known bounded windows at M2: 8 `tests/test_walkthrough_*.mjs`
  still fail to load pending the WP-6A/M6 walker migration; `pub_gamble.spec.mjs`
  interaction specs stay red until WP-5D; the `visual_render.spec.mjs` town
  coverage floor is pending WP-3C recalibration once real facade art ships.
...
- Testing (Patch 15, WP-4C): added 8 entry-state-machine specs to
  ...
  3 skipped (old topology cases awaiting WP-5D re-homing) / 0 failed.
- Testing (Patch 54, WP-5D): rewrote the
  `tests/playwright/town_street.spec.mjs` transaction-panel specs against the
  real outfit and Land Office panels, replacing the retired WP-4A
  ...
  Playwright suite after the WP-5D re-home: 98 passed, 0 failed, 0 skipped
