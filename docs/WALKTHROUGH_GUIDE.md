...
| `hunt_wampus`/`assay_plot` log-and-end fallback | These two opportunistic develop plans have no dedicated spatial executor; the orchestrator logs an info line naming the skipped plan and ends the develop turn via the normal end-turn gesture instead of re-deciding the same move forever. | Both are free, strictly-beneficial scouting moves the develop AI slips in opportunistically, not the turn's committing economic gesture; skipping them costs nothing since the AI will attempt them again next turn if still available, and a dedicated executor for them is a separate, smaller follow-on. |
| Town commerce door-executors walk in rather than key-press | `buy_mule`, `outfit_mule`, and `gamble_pub` x-seek to the target door's street column, wait for `data-door-state="open"`, then press north via `walkTownAvatarNorthUntil` until the door's interaction fires; `walkBackToStreet` returns the avatar to the street row after a successful buy/outfit. | Matches the WP-3B walk-in-triggers-shop door model (no separate action-key press to enter); the pub still keys Enter/Space for the turn-ending gamble CONFIRM dialog, which is a distinct in-dialog confirm, not door entry. |
